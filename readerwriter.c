...
int main() {
    W = sem_open(SEMWRITERS, O_CREAT, S_IRUSR | S_IWUSR, 0);
    R = sem_open(SEMREADERS, O_CREAT, S_IRUSR | S_IWUSR, 0);
    ...
    sem_close(W);
    sem_close (R);
    sem_unlink(SEMWRITERS);
    sem_unlink(SEMREADERS);
}
